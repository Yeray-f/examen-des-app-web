      ...
      <div class="offers-grid">
        <div v-for="oferta in ofertasEspeciales" :key="oferta.id" class="offer-card">
          <div class="offer-cover">
            ...
            <img v-if="oferta.image" :src="oferta.image" :alt="oferta.name" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" />
...
export default {
  ...
  computed: {
    ...
    nuevosLibros() {
      const badges = ['ESPECIAL','POPULAR',null,null,'ESPECIAL','POPULAR',null]
      return this.productos.slice(0, 7).map((p, i) => ({
        ...p, badge: badges[i] || null, rating: (3.8 + (i * 0.15)).toFixed(1)
      }))
    },
    ofertasEspeciales() {
...
.books-row { display: flex; gap: 12px; overflow-x: auto; padding-bottom: 8px; }
