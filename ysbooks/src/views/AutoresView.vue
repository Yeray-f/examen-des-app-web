<template>
  <div>
    ...
    <div v-else class="autores-grid">
      ...
      >
        <!-- Portadas del autor en formato limpio -->
        ...
        <div class="autor-info">
          <div class="autor-avatar-wrap">
            <img :src="autor.foto" :alt="autor.nombre" class="autor-avatar" referrerpolicy="no-referrer" @error="onAvatarError($event, autor.nombre)" />
          </div>
    ...
    <div v-if="autorSeleccionado" class="modal fade show d-block" tabindex="-1" role="dialog" aria-modal="true" @click.self="cerrarModal">
      <div class="modal-autor">
        ...
        <div class="modal-autor-header">
          <img :src="autorSeleccionado.foto" :alt="autorSeleccionado.nombre" class="modal-autor-avatar" referrerpolicy="no-referrer" />
...
// Fotos de autores desde fuentes públicas
const AUTOR_META = {
  'Austin Kleon':       { foto: 'https://fundacionlasgracias.cl/wp-content/uploads/2022/03/austin_kleon.jpeg', bio: 'Austin Kleon es un escritor y artista que vive en Austin, Texas. Es el autor de la trilogía sobre creatividad: Roba como un artista, Muestra tu trabajo y Keep Going.' },
  'Mark Manson':        { foto: 'https://cdn.prod.website-files.com/63b9c43cf803b0331540fa68/6700d4ccdb64bfceedea7786_Mark%20Manson.png', bio: 'Mark Manson es bloguero, emprendedor y autor de superventas del New York Times. Es conocido por su enfoque pragmático y sin filtros sobre la autoayuda y la psicología.' },
  'Agatha Christie':    { foto: 'https://s3.amazonaws.com/arc-wordpress-client-uploads/lanacionpy/wp-content/uploads/2017/11/13190753/AGATHA-CHRISTIE-RECUADRO.jpg', bio: 'Agatha Christie, la "Reina del crimen", fue una novelista inglesa conocida por sus 66 novelas policiales. Creadora de los célebres personajes Hércules Poirot y Miss Marple.' },
  'Paulo Coelho':       { foto: 'https://imagessl.casadellibro.com/img/autores/34590.jpg', bio: 'Paulo Coelho es un novelista brasileño y uno de los autores más vendidos del mundo. Su obra más célebre, El Alquimista, ha vendido más de 150 millones de ejemplares.' },
  'Yuval Noah Harari':  { foto: 'https://imagessl.casadellibro.com/img/autores/w/Harari.webp', bio: 'Yuval Noah Harari es un historiador y escritor israelí. Sus obras Sapiens, Homo Deus y 21 lecciones para el siglo XXI lo han convertido en uno de los pensadores más influyentes.' },
  'Arthur Conan Doyle': { foto: 'https://anikaentrelibros.com/media/154222/arthur-conan-doyle.jpg', bio: 'Sir Arthur Conan Doyle fue un escritor y médico escocés, creador del icónico detective Sherlock Holmes. Sus obras de misterio y aventura son clásicos de la literatura universal.' },
  'Joseph Murphy':      { foto: 'https://images-na.ssl-images-amazon.com/images/I/41VDzk4kdML.jpg', bio: 'Joseph Murphy fue un autor de autoayuda irlando-americano y ministro religioso. Es conocido principalmente por su influyente libro sobre el poder de la mente subconsciente.' },
  'Phil Knight':        { foto: 'https://cdn.britannica.com/18/250018-050-F2A4DFC5/Phil-Knight-Nike-2017.jpg', bio: 'Phil Knight es cofundador y presidente emérito de Nike, Inc. En su autobiografía "Nunca te pares" narra la historia del surgimiento de una de las marcas más icónicas del mundo.' },
}
...
export default {
  ...
  computed: {
    autores() {
      ...
      this.productos.forEach(p => {
        ...
        if (!mapa[authorKey]) {
          // Buscar en AUTOR_META de forma case-insensitive
          const metaKey = Object.keys(AUTOR_META).find(k => k.toLowerCase() === authorKey.toLowerCase())
          const meta = metaKey ? AUTOR_META[metaKey] : {
            foto: '/assets/authors/avatar-fallback.svg',
        ...
        mapa[authorKey].libros.push(p)
        if (!mapa[authorKey].categorias.includes(p.category)) {
          mapa[authorKey].categorias.push(p.category)
        }
      })
  ...
  created() {
    ...
    // Escuchar cambios en localStorage (cuando se elimina/agrega un libro)
    this._storageHandler = () => { this.productos = obtenerProductos() }
    window.addEventListener('storage', this._storageHandler)
    // Escuchar eventos de la misma pestaña
    window.addEventListener('ysbooks-updated', this._storageHandler)
  },
  mounted() {
    this.productos = obtenerProductos()
  },
  beforeUnmount() {
    ...
    window.removeEventListener('ysbooks-updated', this._storageHandler)
  },
  methods: {
...
/* Grid autores */
.autores-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(210px, 1fr)); gap:16px; }
...
.autor-card:hover .cover-strip { opacity:.9; transform: scale(1.02); }
