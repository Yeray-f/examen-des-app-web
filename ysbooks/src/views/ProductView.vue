      ...
      <table class="table table-dark table-hover align-middle ys-table">
        ...
        <tbody>
          ...
          <tr v-for="(producto, index) in productosFiltrados" :key="producto.id">
            ...
            <td>
              <img
                :src="producto.image || '/assets/covers/cover-fallback.svg'"
                :alt="producto.name"
                class="product-thumb"
                @error="onImgError"
              />
      ...
      <div class="modal-box modal-box--detail">
        ...
        <div class="modal-box__body">
          <div class="d-flex gap-3 flex-wrap">
            <img
              :src="productoDetalle.image || '/assets/covers/cover-fallback.svg'"
...
export default {
  ...
  async created() {
    ...
    this._updateHandler = () => { this.productos = obtenerProductos() }
    window.addEventListener('ysbooks-updated', this._updateHandler)
  },
  beforeUnmount() {
    if (this._updateHandler) window.removeEventListener('ysbooks-updated', this._updateHandler)
  },
  methods: {
    // ── LISTAR ──
    cargarProductos() {
    ...
    async reiniciarDatos() {
      if (confirm('¿Reiniciar todos los datos desde el JSON original?')) {
        // Intentar restaurar desde backup local primero
        const restoredFromBackup = reiniciarProductos()
        if (!restoredFromBackup) {
          // Si no hay backup, recargar desde el JSON
          try {
            const response = await fetch('/products.json')
            const productosJson = await response.json()
            forzarInicializacion(productosJson)
          } catch (e) {
