...
export default {
  ...
  computed: {
    ...
    librosFiltrados() {
      ...
      if (this.busqueda) {
        ...
        lista = lista.filter(p => p.name.toLowerCase().includes(q) || p.author.toLowerCase().includes(q))
      }
      if (this.filtroAutor) {
        lista = lista.filter(p => p.author === this.filtroAutor)
      }
      if (this.filtroCategoria) {
        lista = lista.filter(p => p.category === this.filtroCategoria)
      }
      return lista
    }
  },
  created() {
    ...
    this.aplicarFiltrosDesdeRuta(this.$route.query)
  },
  watch: {
    '$route.query': {
      immediate: true,
      handler(q) { this.aplicarFiltrosDesdeRuta(q || {}) }
    }
  },
  methods: {
    aplicarFiltrosDesdeRuta(query) {
      this.busqueda = query.q || ''
      this.filtroCategoria = query.category || ''
      this.filtroAutor = query.author || ''
    },
